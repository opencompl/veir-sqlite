"builtin.module"() ({
  "llvm.mlir.global"() <{addr_space = 0 : i32, alignment = 1 : i64, constant, dso_local, global_type = !llvm.array<8 x i8>, linkage = #llvm.linkage<external>, sym_name = "jsonBlobOverwrite.aType", tls_mode = 0 : i64, unnamed_addr = 0 : i64, visibility_ = 1 : i64}> ({
  }) : () -> ()
  "llvm.module_flags"() <{flags = [#llvm.mlir.module_flag<error, "wchar_size", 4 : i32>, #llvm.mlir.module_flag<min, "PIC Level", 2 : i32>, #llvm.mlir.module_flag<max, "PIE Level", 2 : i32>, #llvm.mlir.module_flag<max, "uwtable", 2 : i32>, #llvm.mlir.module_flag<max, "frame-pointer", 2 : i32>]}> : () -> ()
  "llvm.func"() <{CConv = #llvm.cconv<ccc>, arg_attrs = [{llvm.noundef}, {llvm.noundef}, {llvm.noundef}, {llvm.noundef}], dso_local, frame_pointer = #llvm.framePointerKind<all>, function_type = !llvm.func<i32 (ptr, ptr, i32, i32)>, linkage = #llvm.linkage<external>, no_inline, no_unwind, passthrough = [["min-legal-vector-width", "0"], ["no-trapping-math", "true"], ["stack-protector-buffer-size", "8"], ["target-cpu", "x86-64"]], sym_name = "jsonBlobOverwrite", target_cpu = "x86-64", target_features = #llvm.target_features<["+cmov", "+cx8", "+fxsr", "+mmx", "+sse", "+sse2", "+x87"]>, tune_cpu = "generic", unnamed_addr = 0 : i64, uwtable_kind = #llvm.uwtableKind<async>, visibility_ = 1 : i64}> ({
  ^bb0(%arg0: !llvm.ptr, %arg1: !llvm.ptr, %arg2: i32, %arg3: i32):
    %0 = "llvm.mlir.constant"() <{value = 0 : i64}> : () -> i64
    %1 = "llvm.mlir.constant"() <{value = 15 : i32}> : () -> i32
    %2 = "llvm.mlir.constant"() <{value = 2 : i32}> : () -> i32
    %3 = "llvm.mlir.constant"() <{value = 4 : i32}> : () -> i32
    %4 = "llvm.mlir.constant"() <{value = 0 : i32}> : () -> i32
    %5 = "llvm.mlir.constant"() <{value = 9 : i32}> : () -> i32
    %6 = "llvm.mlir.constant"() <{value = 5 : i8}> : () -> i8
    %7 = "llvm.mlir.constant"() <{value = 6 : i32}> : () -> i32
    %8 = "llvm.mlir.constant"() <{value = 3 : i32}> : () -> i32
    %9 = "llvm.mlir.constant"() <{value = 3 : i8}> : () -> i8
    %10 = "llvm.mlir.constant"() <{value = 1 : i32}> : () -> i32
    %11 = "llvm.mlir.constant"() <{value = 138 : i32}> : () -> i32
    %12 = "llvm.mlir.constant"() <{value = 2 : i8}> : () -> i8
    %13 = "llvm.mlir.constant"() <{value = 278 : i32}> : () -> i32
    %14 = "llvm.mlir.constant"() <{value = 1 : i8}> : () -> i8
    %15 = "llvm.mlir.addressof"() <{global_name = @jsonBlobOverwrite.aType}> : () -> !llvm.ptr
    %16 = "llvm.mlir.constant"() <{value = -1 : i32}> : () -> i32
    %17 = "llvm.mlir.constant"() <{value = 255 : i32}> : () -> i32
    %18 = "llvm.mlir.constant"() <{value = 8 : i32}> : () -> i32
    %19 = "llvm.getelementptr"(%arg1, %0) <{elem_type = i8, noWrapFlags = 3 : i32, rawConstantIndices = array<i32: -2147483648>}> : (!llvm.ptr, i64) -> !llvm.ptr
    %20 = "llvm.load"(%19) <{alignment = 1 : i64, ordering = 0 : i64}> : (!llvm.ptr) -> i8
    %21 = "llvm.zext"(%20) : (i8) -> i32
    %22 = "llvm.and"(%21, %1) : (i32, i32) -> i32
    %23 = "llvm.icmp"(%22, %2) <{predicate = 3 : i64}> : (i32, i32) -> i1
    "llvm.cond_br"(%23)[^bb1, ^bb2] <{operandSegmentSizes = array<i32: 1, 0, 0>}> : (i1) -> ()
  ^bb1:  // pred: ^bb0
    "llvm.br"(%4)[^bb22] : (i32) -> ()
  ^bb2:  // pred: ^bb0
    %24 = "llvm.getelementptr"(%arg1, %0) <{elem_type = i8, noWrapFlags = 3 : i32, rawConstantIndices = array<i32: -2147483648>}> : (!llvm.ptr, i64) -> !llvm.ptr
    %25 = "llvm.load"(%24) <{alignment = 1 : i64, ordering = 0 : i64}> : (!llvm.ptr) -> i8
    %26 = "llvm.zext"(%25) : (i8) -> i32
    %27 = "llvm.ashr"(%26, %3) : (i32, i32) -> i32
    "llvm.switch"(%27)[^bb3, ^bb6, ^bb9, ^bb13, ^bb16] <{case_operand_segments = array<i32: 0, 0, 0, 0>, case_values = dense<[12, 13, 14, 15]> : vector<4xi32>, operandSegmentSizes = array<i32: 1, 0, 0>}> : (i32) -> ()
  ^bb3:  // pred: ^bb2
    %28 = "llvm.shl"(%10, %arg3) <{overflowFlags = 0 : i32}> : (i32, i32) -> i32
    %29 = "llvm.and"(%28, %13) : (i32, i32) -> i32
    %30 = "llvm.icmp"(%29, %4) <{predicate = 0 : i64}> : (i32, i32) -> i1
    "llvm.cond_br"(%30)[^bb4, ^bb5] <{operandSegmentSizes = array<i32: 1, 0, 0>}> : (i1) -> ()
  ^bb4:  // pred: ^bb3
    "llvm.br"(%4)[^bb22] : (i32) -> ()
  ^bb5:  // pred: ^bb3
    %31 = "llvm.add"(%arg3, %10) <{overflowFlags = 0 : i32}> : (i32, i32) -> i32
    "llvm.br"(%31, %14)[^bb17] : (i32, i8) -> ()
  ^bb6:  // pred: ^bb2
    %32 = "llvm.shl"(%10, %arg3) <{overflowFlags = 0 : i32}> : (i32, i32) -> i32
    %33 = "llvm.and"(%32, %11) : (i32, i32) -> i32
    %34 = "llvm.icmp"(%33, %4) <{predicate = 0 : i64}> : (i32, i32) -> i1
    "llvm.cond_br"(%34)[^bb7, ^bb8] <{operandSegmentSizes = array<i32: 1, 0, 0>}> : (i1) -> ()
  ^bb7:  // pred: ^bb6
    "llvm.br"(%4)[^bb22] : (i32) -> ()
  ^bb8:  // pred: ^bb6
    %35 = "llvm.add"(%arg3, %2) <{overflowFlags = 0 : i32}> : (i32, i32) -> i32
    "llvm.br"(%35, %12)[^bb17] : (i32, i8) -> ()
  ^bb9:  // pred: ^bb2
    %36 = "llvm.icmp"(%arg3, %2) <{predicate = 1 : i64}> : (i32, i32) -> i1
    "llvm.cond_br"(%36)[^bb10, ^bb12] <{operandSegmentSizes = array<i32: 1, 0, 0>}> : (i1) -> ()
  ^bb10:  // pred: ^bb9
    %37 = "llvm.icmp"(%arg3, %7) <{predicate = 1 : i64}> : (i32, i32) -> i1
    "llvm.cond_br"(%37)[^bb11, ^bb12] <{operandSegmentSizes = array<i32: 1, 0, 0>}> : (i1) -> ()
  ^bb11:  // pred: ^bb10
    "llvm.br"(%4)[^bb22] : (i32) -> ()
  ^bb12:  // 2 preds: ^bb9, ^bb10
    %38 = "llvm.add"(%arg3, %8) <{overflowFlags = 0 : i32}> : (i32, i32) -> i32
    "llvm.br"(%38, %9)[^bb17] : (i32, i8) -> ()
  ^bb13:  // pred: ^bb2
    %39 = "llvm.icmp"(%arg3, %3) <{predicate = 1 : i64}> : (i32, i32) -> i1
    "llvm.cond_br"(%39)[^bb14, ^bb15] <{operandSegmentSizes = array<i32: 1, 0, 0>}> : (i1) -> ()
  ^bb14:  // pred: ^bb13
    "llvm.br"(%4)[^bb22] : (i32) -> ()
  ^bb15:  // pred: ^bb13
    "llvm.br"(%5, %6)[^bb17] : (i32, i8) -> ()
  ^bb16:  // pred: ^bb2
    "llvm.br"(%4)[^bb22] : (i32) -> ()
  ^bb17(%40: i32, %41: i8):  // 4 preds: ^bb5, ^bb8, ^bb12, ^bb15
    %42 = "llvm.getelementptr"(%arg1, %0) <{elem_type = i8, noWrapFlags = 3 : i32, rawConstantIndices = array<i32: -2147483648>}> : (!llvm.ptr, i64) -> !llvm.ptr
    %43 = "llvm.load"(%42) <{alignment = 1 : i64, ordering = 0 : i64}> : (!llvm.ptr) -> i8
    %44 = "llvm.zext"(%43) : (i8) -> i32
    %45 = "llvm.and"(%44, %1) : (i32, i32) -> i32
    %46 = "llvm.sub"(%40, %2) <{overflowFlags = 0 : i32}> : (i32, i32) -> i32
    %47 = "llvm.zext"(%46) : (i32) -> i64
    %48 = "llvm.getelementptr"(%15, %0, %47) <{elem_type = !llvm.array<8 x i8>, noWrapFlags = 3 : i32, rawConstantIndices = array<i32: -2147483648, -2147483648>}> : (!llvm.ptr, i64, i64) -> !llvm.ptr
    %49 = "llvm.load"(%48) <{alignment = 1 : i64, ordering = 0 : i64}> : (!llvm.ptr) -> i8
    %50 = "llvm.zext"(%49) : (i8) -> i32
    %51 = "llvm.or"(%45, %50) : (i32, i32) -> i32
    %52 = "llvm.trunc"(%51) <{overflowFlags = 0 : i32}> : (i32) -> i8
    %53 = "llvm.getelementptr"(%arg0, %0) <{elem_type = i8, noWrapFlags = 3 : i32, rawConstantIndices = array<i32: -2147483648>}> : (!llvm.ptr, i64) -> !llvm.ptr
    "llvm.store"(%52, %53) <{alignment = 1 : i64, ordering = 0 : i64}> : (i8, !llvm.ptr) -> ()
    %54 = "llvm.zext"(%40) : (i32) -> i64
    %55 = "llvm.getelementptr"(%arg0, %54) <{elem_type = i8, noWrapFlags = 3 : i32, rawConstantIndices = array<i32: -2147483648>}> : (!llvm.ptr, i64) -> !llvm.ptr
    %56 = "llvm.zext"(%41) : (i8) -> i64
    %57 = "llvm.getelementptr"(%arg1, %56) <{elem_type = i8, noWrapFlags = 3 : i32, rawConstantIndices = array<i32: -2147483648>}> : (!llvm.ptr, i64) -> !llvm.ptr
    %58 = "llvm.zext"(%41) : (i8) -> i32
    %59 = "llvm.sub"(%arg2, %58) <{overflowFlags = 0 : i32}> : (i32, i32) -> i32
    %60 = "llvm.zext"(%59) : (i32) -> i64
    "llvm.intr.memcpy"(%55, %57, %60) <{arg_attrs = [{llvm.align = 1 : i64}, {llvm.align = 1 : i64}, {}], isVolatile = false}> : (!llvm.ptr, !llvm.ptr, i64) -> ()
    %61 = "llvm.zext"(%41) : (i8) -> i32
    %62 = "llvm.sub"(%arg2, %61) <{overflowFlags = 0 : i32}> : (i32, i32) -> i32
    "llvm.br"(%62, %40)[^bb18] : (i32, i32) -> ()
  ^bb18(%63: i32, %64: i32):  // 2 preds: ^bb17, ^bb20
    %65 = "llvm.add"(%64, %16) <{overflowFlags = 0 : i32}> : (i32, i32) -> i32
    %66 = "llvm.and"(%63, %17) : (i32, i32) -> i32
    %67 = "llvm.trunc"(%66) <{overflowFlags = 0 : i32}> : (i32) -> i8
    %68 = "llvm.zext"(%65) : (i32) -> i64
    %69 = "llvm.getelementptr"(%arg0, %68) <{elem_type = i8, noWrapFlags = 3 : i32, rawConstantIndices = array<i32: -2147483648>}> : (!llvm.ptr, i64) -> !llvm.ptr
    "llvm.store"(%67, %69) <{alignment = 1 : i64, ordering = 0 : i64}> : (i8, !llvm.ptr) -> ()
    %70 = "llvm.icmp"(%65, %10) <{predicate = 0 : i64}> : (i32, i32) -> i1
    "llvm.cond_br"(%70)[^bb19, ^bb20] <{operandSegmentSizes = array<i32: 1, 0, 0>}> : (i1) -> ()
  ^bb19:  // pred: ^bb18
    "llvm.br"()[^bb21] : () -> ()
  ^bb20:  // pred: ^bb18
    %71 = "llvm.lshr"(%63, %18) : (i32, i32) -> i32
    "llvm.br"(%71, %65)[^bb18] : (i32, i32) -> ()
  ^bb21:  // pred: ^bb19
    "llvm.br"(%10)[^bb22] : (i32) -> ()
  ^bb22(%72: i32):  // 7 preds: ^bb1, ^bb4, ^bb7, ^bb11, ^bb14, ^bb16, ^bb21
    "llvm.return"(%72) : (i32) -> ()
  }) : () -> ()
}) {dlti.dl_spec = #dlti.dl_spec<!llvm.ptr<270> = dense<32> : vector<4xi64>, !llvm.ptr<271> = dense<32> : vector<4xi64>, !llvm.ptr<272> = dense<64> : vector<4xi64>, i64 = dense<64> : vector<2xi64>, i128 = dense<128> : vector<2xi64>, f80 = dense<128> : vector<2xi64>, !llvm.ptr = dense<64> : vector<4xi64>, i1 = dense<8> : vector<2xi64>, i8 = dense<8> : vector<2xi64>, i16 = dense<16> : vector<2xi64>, i32 = dense<32> : vector<2xi64>, f16 = dense<16> : vector<2xi64>, f64 = dense<64> : vector<2xi64>, f128 = dense<128> : vector<2xi64>, "dlti.endianness" = "little", "dlti.mangling_mode" = "e", "dlti.legal_int_widths" = array<i32: 8, 16, 32, 64>, "dlti.stack_alignment" = 128 : i64>, llvm.ident = "Debian clang version 19.1.7 (3+b1)", llvm.module_asm = [], llvm.target_triple = "x86_64-pc-linux-gnu"} : () -> ()
